...
class DayTwo(object):
    def __init__(self):
        with open('two/input.txt', 'r') as reader:
            self.inputs = [self.process_input_line(line) for line in reader]

        self.valid_passwords = self.check_validity()

    def process_input_line(self, reader_object_line: str):
    ...
    def check_password_against_rule(self, line_input: Tuple[int, int, str, str]):
